  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-3 mb-6">
        <button className="p-1.5 text-[#9b9b9b] hover:text-[#0d0d0d] hover:bg-[#f4f4f4] rounded-lg transition-colors" onClick={onCancel}>
          <ChevronLeft size={16} />
        </button>
      ...
      <div className="space-y-5 flex-1 overflow-y-auto">
        ...
        <div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex gap-2 flex-wrap">
              {AVATAR_COLORS.map((c) => (
                <button key={c} className="w-7 h-7 rounded-full border-2 transition-all"
        ...
        <div>
          <label className="block text-sm text-[#6b6b6b] mb-1.5">Name *</label>
          <input className="w-full bg-white border border-[#e5e5e5] rounded-xl px-3 py-2.5 text-sm text-[#0d0d0d] outline-none focus:border-[#b0b0b0] placeholder-[#b0b0b0]"
            value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Code Reviewer" />
        </div>

        <div>
          <label className="block text-sm text-[#6b6b6b] mb-1.5">Description</label>
          <input className="w-full bg-white border border-[#e5e5e5] rounded-xl px-3 py-2.5 text-sm text-[#0d0d0d] outline-none focus:border-[#b0b0b0] placeholder-[#b0b0b0]"
            value={description} onChange={(e) => setDescription(e.target.value)} placeholder="What does this assistant do?" />
        </div>

        <div>
          <label className="block text-sm text-[#6b6b6b] mb-1.5">System Prompt *</label>
          <textarea className="w-full bg-white border border-[#e5e5e5] rounded-xl px-3 py-2.5 text-sm text-[#0d0d0d] outline-none focus:border-[#b0b0b0] placeholder-[#b0b0b0] resize-none"
        ...
        <div>
          <label className="block text-sm text-[#6b6b6b] mb-1.5">Default Model</label>
          <select className="w-full bg-white border border-[#e5e5e5] rounded-xl px-3 py-2.5 text-sm text-[#0d0d0d] outline-none focus:border-[#b0b0b0]"
      ...
      <div className="flex justify-end gap-2 pt-4 border-t border-[#e5e5e5] mt-4 shrink-0">
        <button className="px-4 py-2 text-sm text-[#6b6b6b] hover:text-[#0d0d0d] hover:bg-[#f4f4f4] rounded-xl transition-colors" onClick={onCancel}>
          Cancel
        </button>
        <button
...
function AssistantCard({ assistant, onEdit, onDelete, onChat }: {
  ...
  return (
    <div className="group flex flex-col gap-3 p-4 bg-white hover:bg-[#f9f9f9] rounded-2xl border border-[#e5e5e5] hover:border-[#d1d1d1] transition-all cursor-pointer" onClick={onChat}>
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <button className="p-1.5 text-[#9b9b9b] hover:text-[#0d0d0d] hover:bg-[#f0f0f0] rounded-lg transition-colors"
            onClick={(e) => { e.stopPropagation(); onEdit(); }}><Pencil size={13} /></button>
          <button className="p-1.5 text-[#9b9b9b] hover:text-red-500 hover:bg-[#f0f0f0] rounded-lg transition-colors"
...
export function AssistantsModal() {
  const { assistants, setShowAssistants, createAssistant, updateAssistant, deleteAssistant, createNewChat } = useAppStore();
  const [view, setView] = useState<"list" | "create" | "edit">("list");
  const [editing, setEditing] = useState<Assistant | null>(null);

  const handleCreate = async (data: Omit<Assistant, "id" | "created_at">) => {
    await createAssistant(data.name, data.description, data.system_prompt, data.model, data.provider, data.avatar_color);
    setView("list");
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
      <div className="bg-white border border-[#e5e5e5] rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] flex flex-col overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#e5e5e5] shrink-0">
          ...
          <div className="flex items-center gap-2">
            {view === "list" && (
              <button className="flex items-center gap-1.5 px-3 py-1.5 bg-[#0d0d0d] hover:bg-[#2d2d2d] text-white rounded-xl text-xs font-medium transition-colors"
                onClick={() => setView("create")}>
                <Plus size={13} /> New Assistant
              </button>
            )}
            <button className="p-1.5 text-[#9b9b9b] hover:text-[#0d0d0d] hover:bg-[#f4f4f4] rounded-lg transition-colors" onClick={() => setShowAssistants(false)}>
              <X size={16} />
            </button>
          </div>
        </div>
          ...
          {view === "list" && (
            assistants.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <button className="flex items-center gap-2 px-4 py-2 bg-[#0d0d0d] hover:bg-[#2d2d2d] text-white rounded-xl text-sm font-medium transition-colors"
                  onClick={() => setView("create")}>
              ...
              <div className="grid grid-cols-2 gap-3">
                {assistants.map((a) => (
                  <AssistantCard key={a.id} assistant={a}
                    ...
                    onChat={async () => { await createNewChat(a.model, a.provider, a.id); setShowAssistants(false); }} />
                ))}
              </div>
            )
          )}
